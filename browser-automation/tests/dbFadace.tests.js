...
describe("dbFacade", () => {
  ...
  describe("getDailyConfig()", () => {
    ...
    it("return value should contain properties", (done) => {
      db.getDailyConfig("test", new Date(Date.UTC(2022, 4, 1)))
        .then((dailyConfig) => {
  ...
  describe("getWeeklyConfig()", () => {
    ...
    it("return value should contain properties", (done) => {
      db.getWeeklyConfig("test", new Date(Date.UTC(2022, 7, 1)))
        .then((weeklyConfig) => {
          expect(weeklyConfig).to.deep.equal(
            weeklyConfigPerUser["test"][0].config
          );
          done();
        })
        .catch((err) => done(err));
    ...
    weeklyConfigPerUser["test"].forEach((item) => {
      ...
      it(
        "return value for weekday: " + item.date.getDay() + " should be equal",
        (done) => {
    ...
    it("return value should contain a start_btn action", (done) => {
      db.getWorkweekExceptions("test", new Date(Date.UTC(2022, 7, 8)))
        .then((returnVal) => {
          ...
          const returnedStartBtnExcept = returnVal.filter(
            (val) => val.action === AUTOMATE_ACTION.START_BTN
          )[0];
    ...
    it("return value should contain a stop_btn action", (done) => {
      db.getWorkweekExceptions("test", new Date(Date.UTC(2022, 7, 8)))
        .then((returnVal) => {
          ...
          const returnedStopBtnExcept = returnVal.filter(
            (val) => val.action === AUTOMATE_ACTION.START_BTN
          )[0];
  ...
  describe("getLogEntry()", () => {
    ...
    it("should return an object with all properties", (done) => {
      ...
      db.getLogEntries("test", time)
        .then((logEntries) => {
          ...
          const startBtnLE = logEntries.filter(
            (le) => le.action === AUTOMATE_ACTION.START_BTN
          );
          expect(startBtnLE).to.deep.include(logEntriesPerUser["test"][0]);

          const stopBtnLE = logEntries.filter(
            (le) => le.action === AUTOMATE_ACTION.STOP_BTN
          );
    ...
    it("should return the newly added log entry", (done) => {
      ...
      db.addLogEntry(
        ...
        .then((_) => {
          db.getLogEntries("test", time).then((logEntries) => {
